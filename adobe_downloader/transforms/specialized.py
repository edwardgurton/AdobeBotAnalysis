...
def transform_bot_rule_compare(
    ...
    *,
    ...
    Supports two filename formats generated at different points in time:

    Production format (current, generated by bot_rule_compare.py):
      {client}_{reportType}_{rsid}-{ruleName}-Compare-{ver}-{trafficType}_{[DIMSEGid_hash_]}{from}_{to}
      "-Compare-" appears in parts[2].

    Legacy format (historical, from earlier naming convention):
      {client}_{reportType}_{rsid}_{round}_{ruleName}-Compare-{ver}-{trafficType}_{[DIMSEGid_hash_]}{from}_{to}
      "-Compare-" appears in parts[4].

    The format is detected by checking which underscore-delimited part contains "-Compare-".
    """
    stem = json_path.stem
    parts = stem.split("_")
    ...
    if "-Compare-" in parts[2]:
        # Production format: all of rsid, rule name, version, and traffic type are
        # hyphen-delimited within parts[2], e.g. "SBRcom-SG-GeoCountry-Compare-V1-AllTraffic"
        pre, post = parts[2].split("-Compare-", 1)
        pre_hyphen = pre.split("-")
        rsid_name = pre_hyphen[0]
        bot_rule_name = "-".join(pre_hyphen[1:])
        post_parts = post.split("-")
        compare_version = post_parts[0]
        traffic_type = post_parts[1] if len(post_parts) > 1 else ""
        is_segment = traffic_type == "Segment"
        is_compare = traffic_type == "AllTraffic"

        if is_segment and len(parts) > 3 and parts[3].startswith("DIMSEG"):
            # "DIMSEGs3938" + "_" + "67efce1e..." reconstructs the full segment ID
            dim_core = parts[3].replace("DIMSEG", "")
            if len(parts) > 4 and len(parts[4]) == 24:
                segment_hash = parts[4]
                segment_id = f"{dim_core}_{segment_hash}"
        ...
        # Legacy format: rsid in parts[2], period label in parts[3], compare info in parts[4]
        rsid_name = parts[2].split("-")[0]
        complex_parts = parts[4].split("-")
