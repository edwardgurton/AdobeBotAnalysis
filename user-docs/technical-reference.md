...
`iterate_segments` yields `SegmentIteration(ids, id=None, name=None)` objects (`report_download.py`), not bare tuples:

- `None` — yields one `SegmentIteration(ids=[])` (no segment filter, no filename suffix).
- `source="inline"` — yields one `SegmentIteration(ids=ids)` (all IDs in one request, no filename suffix).
- `source="segment_list_file"` — reads a JSON file that is a list of `{"id": ..., "name": ...}` objects. Yields one `SegmentIteration(ids=[seg_id], id=seg_id, name=sanitized_name)` per entry, so each segment gets its own request and, by default, a distinct output filename derived from its sanitized name (see Output file naming below). `load_segment_list` validates eagerly, before any downloads start: every entry must have a non-blank `name`, and no two entries may sanitize to the same filename component (`sanitize_segment_name_for_filename`) — either condition raises `ValueError` immediately, since silently proceeding would let two segments' downloads overwrite the same output file with the state DB still recording both as completed.
...
**`bot_validation`**: `requestName = parts[1]` always. Scans the remaining parts for one starting with (or containing `-` immediately before) `RULE` — the default shape for `segment_list_file`-sourced downloads (see Output file naming above). If found, everything after `RULE` in that part is the real per-rule `botRuleName`, and `rsidName = parts[2]`. If no `RULE` anchor is present — no segment iteration, or `include_segment_id_in_filename`/no per-segment name available — falls back to the legacy fixed-position parse: `botRuleName = parts[2]`, `rsidName = parts[3]`, where `botRuleName` is just whatever static `file_name_extra` batch label the job used, not a genuine per-rule name. Loads headers from the YAML for `requestName`. For each row appends: `fileName, requestName, botRuleName, rsidName`. These four columns must be present at the end of the header YAML definition.
...
A `transform_concat` step with `transform.type: bot_rule_compare` or `transform.type: bot_validation` can set `transform.split_by_bot_rule: true` to produce one CSV per bot rule instead of a single combined file. It auto-detects the bot rule list from the sibling `bot_rule_compare`/`report_download` step's `bot_rules` source in the same composite job (no separate list needed), then calls `concatenate_csv_files` once per rule with the subset of `csv_paths` whose name matches that rule (`_match_csvs_for_bot_rule`) — tried in order: first the `RULE{sanitized_name}` anchor (the default `report_download`/`bot_validation` scheme), falling back to a bare sanitized-name match (`bot_rule_compare`'s legacy scheme, unchanged) if the anchor isn't found. Both are boundary-anchored so a shorter rule name can't match as a substring of a longer one (e.g. `RuleA` no longer matches inside `RuleA2`), writing `{prefix}_{job_name}_{ruleName}.csv` per rule. Step output uses `concatenated_files` (a dict of rule name → path) instead of `concatenated_file` in this mode.
