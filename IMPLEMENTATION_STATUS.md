...
**Last commit:** `Add cleanup job, clean command, and docs` — a standalone disk-cleanup job for the output tree, separate from the download pipeline; 660 tests passing

**Next concrete action:** Run `adobe-downloader clean --config jobs/examples/legend_cleanup.yaml` (dry run), review the report, then set `dry_run: false` and re-run with `--confirm` to reclaim ~5.8 GB on Legend.
